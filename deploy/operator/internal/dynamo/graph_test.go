...
func TestGenerateGrovePodCliqueSet_GMSPodsAreNotCheckpointTargets(t *testing.T) {
	...
	for _, clique := range got.Spec.Template.Cliques {
		...
		if strings.Contains(clique.Name, "gms") {
			...
			for i := range clique.Spec.PodSpec.Containers {
				assert.NotEqual(t, []string{"sleep", "infinity"}, clique.Spec.PodSpec.Containers[i].Command,
					"GMS clique %q container %q must not be rewritten to sleep infinity", clique.Name, clique.Spec.PodSpec.Containers[i].Name)
			}
		} else {
			sawEngine = true
