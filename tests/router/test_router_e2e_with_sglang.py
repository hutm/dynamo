...
SGLANG_ARGS: Dict[str, Any] = {
    "page_size": PAGE_SIZE,
    "model": MODEL_NAME,
    "max_total_tokens": 2048,  # matches requested_sglang_kv_tokens(2048)
    ...
    # Preserve main's memory-safe default while allowing CUDA-graph coverage.
...
class SGLangProcess(ManagedEngineProcessMixin):
    ...
    def __init__(
        ...
        for worker_idx in range(num_workers):
            ...
            # Prefer the explicit token budget; it avoids allocator-dependent
            # device-block counts in multi-worker tests.
            if max_total_tokens is not None:
                command.extend(
                    [
                        "--max-total-tokens",
