    ...
    Resp: Data + HasTokenIds,
{
    ...
    /// Whether the current stream has produced backend output.
    current_stream_has_output: bool,
    /// Process-wide permit held for the lifetime of a migrated stream.
    migration_retry_permit: Option<OwnedSemaphorePermit>,
    /// Failover-only permit held until the replayed stream finishes.
    failover_retry_permit: Option<OwnedSemaphorePermit>,
    /// Permit one bounded replay when an established pre-output stream stops.
    allow_pre_output_stopped_retry: bool,
    /// Permit replay of cancellation-like backend failures during failover.
    allow_failover_stopped_retry: bool,
    /// Whether the current stream was created by failover replay.
    current_stream_from_failover_replay: bool,
    /// Avoid repeatedly cancelling an established replay on first-token timeout.
...
struct MigrationCause {
    ...
    /// The attempt that failed, not the retry it may schedule.
    attempt: u32,
}
    ...
    Resp: Data + HasTokenIds,
{
        ...
        loop {
                ...
                match timeout(migration_first_chunk_timeout(), response_stream.next()).await {
                    ...
                    Err(_) => {
                        if recently_observed_failover() {
                            ...
                            let event =
                                MigrationEvent::new(frontend_service::migration_type::NEW_REQUEST);
                            if let Err(err) = self.new_stream(Some(event)).await {
            ...
            if let Some(response) = response {
                if let Some(err) = response.error.as_ref() {
                    ...
                    if is_migratable_for_request(&self.request, err) || failover_cancel_replayable {
                        ...
                        if self.retries_left == 0 {
                            ...
                            self.queue_migration(err.error_type(), route_trace);
                        }
                        ...
                        if let Err(retry_err) = self.new_stream(Some(event)).await {
                            tracing::warn!(error = ?retry_err, "Cannot recreate stream");
                            self.current_stream_has_output = true;
                        } else {
                            continue;
                        }
            ...
            if !self.current_stream_has_output {
                if self.retries_left > 0 {
                    ...
                    self.metrics.inc_migration_new_request(&self.model_name);
                    self.allow_pre_output_stopped_retry = true;
                    let event = MigrationEvent::new(frontend_service::migration_type::NEW_REQUEST);
                    if let Err(err) = self.new_stream(Some(event)).await {
    ...
    async fn new_stream(&mut self, mut migration_event: Option<MigrationEvent>) -> Result<()> {
        ...
        while self.retries_left > 0 {
            ...
            loop {
                ...
                let route_trace = attach_route_trace_context(
                    ...
                    RouteTraceContext::new(
                        attempt,
                        migration.map(|cause| cause.reason),
                        migration.and_then(|cause| cause.from_worker_id),
                ...
                if context_killed
                    || (context_stopped && !(allow_stopped_retry || allow_failover_stopped_retry))
                {
                    let outcome = frontend_service::migration_outcome::CANCELLED;
                ...
                match self.next_generate.generate(request).await {
                    Ok(next_stream) => {
                        self.record_migration_outcome(
                            migration_event.as_ref(),
                        ...
                        response_stream = Some(Ok(next_stream));
                        break;
                    }
                    Err(err) => {
                        let failover_cancel_replayable =
                            is_failover_cancel_replayable(err.as_ref());
                        if !(is_migratable_for_request(&self.request, err.as_ref())
                            || failover_cancel_replayable)
                        {
            ...
            if response_stream.as_ref().is_some_and(Result::is_ok) {
                break;
            }
            if let Some(Err(err)) = response_stream.as_ref()
    ...
    fn track_response(&mut self, response: &Annotated<Resp>) {
        ...
        // Telemetry must count the final attempt even when no retry remains.
        ...
        self.request.token_ids.extend_from_slice(token_ids);
    }
