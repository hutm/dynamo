...
1. **Memory Manager** - Allocates physical GPU memory via CUDA VMM (`cuMemCreate`). Regular allocation exports are created lazily and transferred as ephemeral per-client file descriptors; clients own their virtual mappings. The persistent-allocation manager additionally maps persistent KV in the daemon for direct access and storage-tier operations. Allocation requests retry on OOM until they succeed or the optional retry timeout is reached.
